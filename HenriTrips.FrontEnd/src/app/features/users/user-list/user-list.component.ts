...
//                          (click)="deleteUser(user)" 
//                          [disabled]="isDeleting === user.id || user.role === 'admin'">
//                    <span class="material-symbols" *ngIf="isDeleting !== user.id">delete</span>
//                    <span class="spinner" *ngIf="isDeleting === user.id"></span>
//                    {{ isDeleting === user.id ? '' : '' }}
//                  </button>
//                </div>
//              </td>
...
//      // IMPORTANT: SUBSCRIBE to the observable to send the HTTP request
//      this.userService.deleteUser(user.id).subscribe({
//        next: (response) => {
//          console.log('User deleted successfully:', response);
//          this.isDeleting = null;
